...
protected:
    ...
    static void duoUtilityQValueCompare(double u1, double u2, double u3, double u4, int expectedResult, MDP *mdp, bool useRanks=false) {
        auto qv1 = BuildUtilityQValue({u1,u2});
        auto qv2 = BuildUtilityQValue({u3,u4});
    ...
    static void trioUtilityQValueCompare(double u1, double u2, double u3, double u4, double u5, double u6, int expectedResult, MDP *mdp, bool useRanks=false) {
        auto qv1 = BuildUtilityQValue({u1,u2,u3});
        auto qv2 = BuildUtilityQValue({u4,u5,u6});
        int actual = mdp->CompareByTheories(qv1, qv2, useRanks);
        EXPECT_EQ(actual, expectedResult)
            << "Comparison fail. (" << qv1.toString() << ") to (" << qv2.toString() << ") -> " << actual << " and not expected " << expectedResult;
    }
