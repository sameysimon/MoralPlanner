...
def Build(fileName:str, theories=1, numOfActions=3, horizon=5, minBranches=3, maxBranches=10, numOfRanks=1):
    transitions = {}
    stateTime = []
    ...
    # Create random tree with horizon, random branches and actions.
    totalStates = 1
    st = [[0, 0]]
    while (len(st)!=0):
        [state, time] = st.pop()
        if (time==horizon):
            continue
        myActions = list(range(np.random.random_integers(1,numOfActions)))
        transitions[str(state)] = {}
        for aIdx in myActions:
            solutions[aIdx]["Action_Map"][str(state)] = str(aIdx)
            ...
            for i in range(numOfBranches):
                st.append([totalStates, time+1])
                transitions[str(state)][str(aIdx)].append(probs[i], totalStates)
                totalStates += 1
                stateTime.append(time + 1)
